...
var baesite = ['', '','http://www.17xiuwang.com/'];
...
var ruleName = ['configlist','redirectlist','refererslist','proxylist', 'urllist'];
...
var compatible = 1;	//用于控制是否启动代理控制,1为禁用,0为启用,initRules过程中将会改变并使用localStorage[]存取该值
...
//		console.log(id);
	...
	for (var i = 0; i < disableurl.length; i++) {
		var find = disableurl[i].find;
		if (find.test(testUrl)) {
			var dnewUrl = "http://" + disableurl[i].replace;
			dnewUrl = decodeURIComponent(dnewUrl);
			return {
				redirectUrl: dnewUrl
			};
		};
	...
	for (var i = 0; i < redirectlist.length; i++) {
		if (localStorage["VideoDisable"] == "false") continue;//新增判断
	...
	//hello new
	if (localStorage["VideoDisable"] == "false" || type == "main_frame") return;
...
youkuplayerswf=baesite[2]+"/swf/player.swf";  
...
a.open("GET", youkuplayerswf.split("").join(""), !0); 
a.send(null) 
} 
...
function initRules(){
	...
	disable = 1;	//开始更新过程
	chrome.storage.local.get('disableurl', function(items) {
		if (items['disableurl'] != null) {
			disableurl = JSON.parse(decode64(items['disableurl']));
			for (var i = 0; i < disableurl.length; i++) {
				disableurl[i].find = new RegExp(disableurl[i].find, "i");
			};
		};
	});
