require.scopes["prefs"] = (function() {
	...
	var defaults = {
		...
		subscriptions_exceptionsurl: "http://tools.17xiuwang.com/adrules.html",
		documentation_link: "https://adblockplus.org/redirect?link=%LINK%&lang=%LANG%",
		notificationdata: {},
		notificationurl: "http://tools.17xiuwang.com/notification.json",
		stats_total: {},
	};
	var listeners = [];
	...
	Downloader.prototype = {
		...
		_download: function(downloadable, redirects) {
			...
			var errorCallback = function errorCallback(error) {
				...
				Cu.reportError("广告净化器: Downloading URL " + downloadable.url + " failed (" + error + ")\n" + "Download address: " + downloadURL + "\n" + "Channel status: " + channelStatus + "\n" + "Server response: " + responseStatus);
...
require.scopes["filterStorage"] = (function() {
	...
	var FilterNotifier = require("filterNotifier").FilterNotifier;
	var formatVersion = 4;
	var FilterStorage = exports.FilterStorage = {get formatVersion() {
		...
		get sourceFile() {
			...
			if (!file) {
				Cu.reportError("广告净化器: Failed to resolve filter file location from extensions.adblockplus.patternsfile preference");
			}
		...
		_generateFilterData: function(subscriptions) {
			...
			_generatorResult16.push("# Adblock Light preferences");
...
require.scopes["filterListener"] = (function() {
	...
	var batchMode = false;
	var isDirty = 0;
	var FilterListener = exports.FilterListener = {get batchMode() {
			return batchMode;
		},
		set batchMode(value) {
			batchMode = value;
