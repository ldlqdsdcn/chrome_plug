...
function init() {
    ...
    $("#footer-left-OptionsButton").click(onOption);
    $("#footer-video").focus();
...
function toggleEnabled() {
    ...
    if (checked) {
        localStorage["VideoDisable"] = true;
...
function updateStats() {
    ...
    if (blockedTotal != null) {
        var st = document.getElementById("statsTotal");
        if (st != null) {
            st.innerHTML = blockedTotal;
            numSkin('.JS-num-set', '.JS-num-val');
        }
        ;
    }
    ;
    if (backgroundPage.GetTabid != null) {
        var blockedpage = getStats("blocked", backgroundPage.GetTabid).toLocaleString();
        var sp = document.getElementById("statspage");
        if (sp != null) {
            sp.innerHTML = blockedpage;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById("whitecheck").addEventListener('click', function () {
        whitepage(this.checked);
    });
});
...
function filltable(data, isMedia) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (isMedia) {
            ...
            newcell.firstChild.addEventListener('click', function () {
                downloadMedia(this.name, this.alt);
            });
            newcell = newrow.insertCell(3);
            newcell.style.cssText = "text-align:right;";
